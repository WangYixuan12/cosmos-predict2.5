...
class CustomDataset(Dataset):
    def __init__(
        ...
        if "bimanual_rope" in train_annotation_path:
            self.action_dim = 8
            self.c_act_scaler = [20.0, 20.0, 20.0, 1.0, 20.0, 20.0, 20.0, 1.0]
        elif "bimanual_box" in train_annotation_path:
            self.action_dim = 14
            self.c_act_scaler = [1.0] * 14
        elif "bimanual_sweep" in train_annotation_path:
            self.action_dim = 4
            self.c_act_scaler = [20.0] * 4
        elif "pusht_1000" in train_annotation_path:
            self.action_dim = 4
            self.c_act_scaler = [20.0] * 4
        elif "sim_aloha" in train_annotation_path:
            self.action_dim = 4
            self.c_act_scaler = [20.0] * 4
        elif "single_chain_in_box" in train_annotation_path:
            self.action_dim = 4
            self.c_act_scaler = [20.0, 20.0, 20.0, 1.0]
        elif "single_grasp" in train_annotation_path:
            self.action_dim = 4
            self.c_act_scaler = [20.0, 20.0, 20.0, 1.0]
        else:
            raise ValueError("unseen task")
        self.c_act_scaler = np.array(self.c_act_scaler, dtype=float)
        self.ann_files = self._init_anns(self.data_path)
        self._filter_rollout()
    ...
    def _get_actions(self, label, frame_ids):
        # print(label)
        all_actions = np.array(label[self._action_key])
        action = all_actions[frame_ids]
        # print(frame_ids)
        return torch.from_numpy(action[1:])  # (l - 1, act_dim)
